...
def getSpectogram(file_path, emotion_label):
    ...
    output_image_path = f"./images9000/{emotion_label}/{processed_path}.png"  # TODO: figure out naming conventions for the file -- either use path or just have a counter that we pass in
...
def traverse_audio_files(directory="./train_splits_wav"):
    ...
    for file_name in os.listdir(directory):
        ...
        if os.path.isfile(file_path) and file_name.endswith('.wav'):
            ...
            data.append({"Transcription": transcription, "Spectogram": image_path, "Emotion": emotion, "Sentiment": sentiment, "Utterance": utterance})
        iterCount += 1
...
df.to_csv('data9000_senti.csv', index=False)